...
function ActionList{AT, V}() where {AT, V}
	ActionList{AT, V}(Dict{AT, Int}(), Float64[], [], 0)
end

sum_rates(a::AgentActions{AT, V}) where {AT, V} = sum(a.rates)
...
function change_rates!(agent::T, alist::ActionList{T, V}, rates::V) where {T,V}
	...
	# change in rate
	delta = sum(rates) - sum_rates(alist.actions[idx])
